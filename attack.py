"""
Purpose: Will generate an audio adversarial attack on the speech to text
model.
"""
...
class Attack:
    def __init__(self, sess, loss_fn, phrase_length, max_audio_len,
        ...
        # clobber them.
        self.delta = delta = torch.zeros(batch_size, max_audio_len, dtype=torch.float32)
        self.mask = mask = torch.zeros(batch_size, max_audio_len, dtype=torch.float32)
        self.cwmask = cwmask = torch.zeros(batch_size, phrase_length, dtype=torch.float32)
        self.original = original = torch.zeros(batch_size, max_audio_len, dtype=torch.float32)
        self.lengths = lengths = torch.zeros(batch_size, dtype=torch.int32)
        self.importance = torch.zeros(batch_size, phrase_length, dtype=np.float32)
        self.target_phrase = torch.zeros(batch_size, phrase_length, dtype=torch.int32)
        self.target_phrase_lengths = torch.zeros(batch_size, dtype=torch.int32)
        self.rescale = torch.zeros(batch_size, 1, dtype=torch.float32)
        ...
        self.apply_delta = torch.clamp(delta, -2000, 2000) * self.rescale
        ...
        noise = torch.empty(new_input.shape).normal_(std=2.)

        #pass_in = tf.clip_by_value(new_input + noise, -2 ** 15, 2 ** 15 - 1)
        pass_in = torch.clamp(new_input + noise, -2 ** 15, 2 ** 15 - 1)

        # Feed this final value to get the logits.
        # TODO: find out what get_logits does
        self.logits = logits = get_logits(pass_in, lengths)

        # And finally restore the graph to make the classifier
        # actually do something interesting.
        # TODO: What is the pytorch equivalent
        ...
        if loss_fn == "CTC":
            ...
            # Pytorch example: ctc_loss(input, target, input_lengths, target_lengths) TODO: maybe figure out if target_lengths is needed
            ctcloss = torch.nn.CTCLoss()(logits, target, lengths)
            ...
            if not np.isinf(l2penalty):
                ...
                loss = torch.mean((self.new_input - self.orginal) ** 2, dim=1) + l2penalty * ctcloss
            else:
                loss = ctcloss
            # self.expanded_loss = tf.constant(0)
            self.expanded_loss = torch.tensor(0, dtype=torch.int32)
    ...
    def attack(self, audio, lengths, target, finetune=None):
        ...
        self.orginal = np.array(audio)
        self.lengths = (np.array(lengths) - 1) // 320
        self.mask = np.array([[1 if i < l else 0 for i in range(self.max_audio_len)] for l in lengths])
        self.cwmask = np.array([[1 if i < l else 0 for i in range(self.phrase_length)] for l in (np.array(lengths) - 1) // 320])
        self.target_phrase_lengths = np.array([len(x) for x in target])
        self.target_phrase = np.array([list(t) + [0] * (self.phrase_length - len(t)) for t in target])
        c = np.ones((self.batch_size, self.phrase_length))
        self.importance = c
        self.rescale = np.ones((self.batch_size, 1))
        ...
        for i in range(MAX):
            ...
            if i % 10 == 0:
                ...
                new, delta, r_out, r_logits = self.new_input, self.delta, self.decoded, self.logits
                lst = [(r_out, r_logits)]
                if self.mp3:
                    ...
                    mp3_out, mp3_logits = (self.decoded, self.logits), {self.new_input: mp3ed}
            ...
            d, el, cl, l, logits, new_input, _ = sess.run((self.delta, self.expanded_loss,
                                                           ...
                                                           self.train),
                                                          feed_dict)
            # TODO: figure out how to pass in feed_dict
            d, el, cl, l, logits, new_input, _ = self.delta, self.expanded_loss, self.ctcloss, self.loss, self.logits, self.new_input, self.train
...
def main():
    ...
    # TODO: get rid of sess in Attack params
    sess = None
